...
This time around I went with **React** for the main application shell and routing, and **Vue.js** for interactive components like the photo gallery and this blog editor.

The design preserves what I had before - Manrope font, clean black-and-white aesthetic, and alternating gallery layout - but now it's a proper SPA with client-side routing.
...
function getPostSlug(post, index = 0) {
  return post.slug || post.id || `post-${index}`
}
...
export default function Blog() {
  ...
  if (slug) {
    const post = posts.find((p, index) => getPostSlug(p, index) === slug)
      ...
      <div className="blog-posts">
        {posts.map((post, i) => (
          <article key={post.slug || i} className="blog-card">
            <h2>
              <Link to={`/blog/${getPostSlug(post, i)}`}>{post.title}</Link>
            </h2>
            <time className="post-date">{post.date}</time>
            <p>{post.excerpt}</p>
            <Link to={`/blog/${getPostSlug(post, i)}`} className="read-more">Read more &rarr;</Link>
          </article>
        ))}
      </div>
    </section>
